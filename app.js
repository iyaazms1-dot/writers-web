...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderPost(post) {
    ...
    authorLine.className = 'post-meta';
    authorLine.innerHTML = `<div class="left"><span class="avatar">${post.author[0]}</span><strong>${post.author}</strong></div><div>${new Date(post.timestamp).toLocaleString()}</div>`;

    const bodyLine = document.createElement('div');
    bodyLine.className = 'post-body';
    bodyLine.textContent = post.text;

    const actionsLine = document.createElement('div');
    actionsLine.className = 'post-actions';
    ...
    actionsLine.appendChild(commentBtn);
    actionsLine.appendChild(shareBtn);
  ...
  // --------------------------
  // Handle post link from URL
  const urlParams = new URLSearchParams(window.location.search);
  const postId = urlParams.get('post');
  if (postId) {
    const post = posts.find(p => p.id == postId);
    if (post) {
      alert(`Post by ${post.author}:\n\n${post.text}`);
    }
  }
